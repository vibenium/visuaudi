/// Used for filtering the input data (avg. between pixel RGB values)
fn mvn_avg_filt(num_points: usize, input_vec: &[f32]) -> Option<Vec<f32>> {
    if num_points < input_vec.len() {
        ...
        // Set up 1st element
        for input_nums in input_vec.iter().take(num_points) {
            // O(1)
            output_vec[0] += input_nums;
        }
        output_vec[0] /= points;
        ...
        for i in 1..(input_vec.len() - num_points) {
            // O(n)
            output_vec.push(
                output_vec[i - 1] - (input_vec[i - 1] - input_vec[(i + num_points) - 1]) / points,
            );
        }
...
pub fn img_to_vec(path: String, num_pts: Option<String>) -> Result<Vec<f32>, &'static str> {
    ...
    if let Some(f) = file {
        ...
        Err("Error: Failed to open image file")
    }
    /*
    if file.is_none() {
        return Err("Failure 1");
    } else {
        let rgb = file.unwrap().into_rgba8();
        let pixels = rgb.pixels();
